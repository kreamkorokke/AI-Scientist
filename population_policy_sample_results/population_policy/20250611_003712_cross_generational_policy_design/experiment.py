...
class PolicyEffectsCalculator:
    ...
    def __init__(self, sensitivity: float = 1.0):
        ...
        # Policy effect matrices for cross-generational interventions
        # Enhanced effects due to synergies between age groups
        # Values >1.0 indicate positive effects, <1.0 indicate negative effects
        ...
        self.fertility_effects = {
            ...
            'tax_incentive_families': [0, 0, 0, 1.05, 1.15, 1.20, 1.15, 1.10, 1.05] + [1.0] * 12,  # Stronger incentives for multi-gen businesses
            'work_life_balance_index': [0, 0, 0, 1.04, 1.18, 1.22, 1.15, 1.08, 1.02] + [1.0] * 12,
            'housing_affordability': [0, 0, 0, 1.12, 1.25, 1.28, 1.20, 1.15, 1.10, 1.05, 1.03, 1.02] + [1.0] * 9,  # Enhanced for multi-gen housing
        ...
        self.mortality_effects = {
            'elder_care_capacity': [1.0] * 10 + [0.92, 0.88, 0.85, 0.82, 0.80, 0.78, 0.75, 0.72, 0.70, 0.68, 0.65],  # Enhanced elder care through proximity
            'education_investment': [0.97, 0.97, 0.97, 0.96, 0.96, 0.96, 0.97, 0.97, 0.98] + [0.98] * 12,  # Enhanced through skill transfer
        ...
        self.migration_effects = {
            ...
            'work_life_balance_index': [1.0, 1.0, 1.10, 1.25, 1.20, 1.15, 1.10, 1.05, 1.02] + [1.0] * 12,  # Enhanced through economic partnerships
...
class PopulationExperiment:
    ...
    def generate_policy_scenarios(self) -> Dict[str, np.ndarray]:
        ...
        scenarios = {
            ...
            # Integrated cross-generational approach
            'integrated_approach': np.clip(baseline + np.array([0.5, 0.5, 0.6, 0.4, 0.6, 0.6, 0.7, 0.7, 0.7, 0.8]), 0, 1),
            
            # Previous successful policies
            'economic_partnership': np.clip(baseline + np.array([0.4, 0.4, 0.3, 0.4, 0.6, 0.5, 0.7, 0.5, 0.4, 0.4]), 0, 1),
            'skill_transfer': np.clip(baseline + np.array([0.2, 0.3, 0.5, 0.3, 0.3, 0.4, 0.3, 0.6, 0.2, 0.8]), 0, 1),
            'three_gen_housing': np.clip(baseline + np.array([0.3, 0.3, 0.4, 0.2, 0.4, 0.5, 0.4, 0.3, 0.7, 0.3]), 0, 1),
