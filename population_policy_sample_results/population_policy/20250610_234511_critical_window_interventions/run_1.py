...
class PopulationExperiment:
    ...
    def generate_policy_scenarios(self) -> Dict[str, Union[np.ndarray, Dict]]:
        ...
        # Early intervention scenario (2024-2029)
        early_intervention_intense = np.array([0.9, 0.9, 0.9, 0.8, 0.8, 0.7, 0.8, 0.9, 0.8, 0.8])  # Aggressive early policies
        early_intervention_sustained = np.array([0.6, 0.6, 0.6, 0.5, 0.5, 0.5, 0.5, 0.6, 0.5, 0.5])  # Moderate sustained support
        
        # Time-varying policies for early intervention
        early_intervention_timeline = {}
        # First 5 years: Intense intervention
        for year in range(5):
            early_intervention_timeline[year] = early_intervention_intense
        # After 5 years: Sustained support
        for year in range(5, 30):
            early_intervention_timeline[year] = early_intervention_sustained
        
        scenarios = {
            'baseline': baseline,
            'early_intervention': {'base_policy': baseline, 'time_varying': early_intervention_timeline}
        }
